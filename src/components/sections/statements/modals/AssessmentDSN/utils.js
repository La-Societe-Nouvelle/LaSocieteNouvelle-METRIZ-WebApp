...
export const getGenderWageGap = async (individualsData) => {
  ...
  // Gander gap
  let genderWageGap = roundValue((Math.abs(hourlyRateMen - hourlyRateWomen) / hourlyRateAll) * 100, 1);
...
export const getIndividualWorkingHours = (individu, regexPeriod) => 
{
  ...
  let isExecutive = isIndividualExecutive(individu);
  if (heuresDeclaration == 0 && isExecutive) {
    return roundValue(1806/12, 2); 
    // 1806 heures -> données INSEE (nombre d'heures moyen contrats cadres)
  }
...
/** Contrat cadre
 *  Conditions -> statut conventionnel :
...
// Cadre ou Non-Cadre
export const isIndividualExecutive = (individu) =>
{
  let isIndividualExecutive = individu.contrats
    .some(contrat => contrat.statutConventionnel=="03" || contrat.statutConventionnel=="04");
  return isIndividualExecutive;
}
