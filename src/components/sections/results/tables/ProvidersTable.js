  ...
  const sortAccounts = (accounts, columnSorted) => {
    switch (columnSorted) {
        ...
        accounts.sort(
          (a, b) => {
            if (a.periodsData[period.periodKey] && b.periodsData[period.periodKey]) {
              return b.periodsData[period.periodKey].amountExpenses - a.periodsData[period.periodKey].amountExpenses;
      ...
      case "impact":
        accounts.sort(
          (a, b) => {
            if (a.footprint.indicators[indic].getGrossImpact(a.periodsData[period.periodKey].amountExpenses) 
              > b.footprint.indicators[indic].getGrossImpact(b.periodsData[period.periodKey].amountExpenses)) {
              return -1;
            } else {
              return 1;
            }
