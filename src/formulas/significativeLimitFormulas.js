...
export async function getSignificativeCompanies(companies,expenses,investments,minFpt,maxFpt)
{
  ...
  let definedCompanies = companies.filter(company => company.state == "siren" && Object.keys(expensesByCompanies).includes(company.account));
  let undefinedCompanies = companies.filter(company => company.state != "siren" && Object.keys(expensesByCompanies).includes(company.account));

  let significativeCompaniesForExpenses = [];
  for (let indic of Object.keys(metaIndics)) {
    let significativeCompaniesForExpensesByIndic = await getSignificativeCompaniesByIndic(indic,definedCompanies,undefinedCompanies,expensesByCompanies,minFpt,maxFpt,limit); // return list accounts
