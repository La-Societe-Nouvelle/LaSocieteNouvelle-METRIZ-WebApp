...
import { Aggregate, buildAggregateFromAccounts, buildAggregateFromItems } from './accountingObjects/Aggregate';
...
export class FinancialData {
    ...
    constructor(data) 
    {
        ...
        // ---------------------------------------------------------------------------------------------------- //
            ...
            this.expenses = data.expenses.map((props,index) => new Expense({id: index, ...props}));                                                         // external expenses (#60[^3], #61, #62)
            
            // Stocks ---------------------------------- //
            
            this.stocks = Object.values(data.stocks).map((props,index) => new Stock({id: index, ...props}));                                                // stocks (#31 to #35, #37) / Depreciation (#29)
            this.stockVariations = data.stockVariations.map((props,index) => new StockVariation({id: index, ...props}));                                    // stock variation (#603, #71)
            
            // Immobilisations ------------------------- //
            
            this.immobilisations = Object.values(data.immobilisations).map((props,index) => new Immobilisation({id: index, ...props}));                     // immobilisations (#20 to #27) / Amortisation (#28) / Depreciation (#29)
            this.amortisationExpenses = data.amortisationExpenses.map((props,index) => new AmortisationExpense({id: index, ...props}));                     // amortisation expenses (#6811, #6871)
            this.adjustedAmortisationExpenses = data.adjustedAmortisationExpenses.map((props,index) => new AmortisationExpense({id: index, ...props}));     // amortisation expenses (#6811, #6871)       
            this.investments = data.investments.map((props,index) => new Expense({id: index, ...props}));                                                   // investments (flows #2 <- #404)
            this.immobilisedProductions = data.immobilisedProductions.map((props,index) => new immobilisedProduction({id: index, ...props}));               // productions of immobilisations (flows #2 <- #72)
            ...
            this.providers = data.providers.map((props,id) => new Provider({id: id, ...props}));
            
            // Aggregates ------------------------------ //
            
            this.mainAggregates = {};
            Object.entries(data.aggregates).forEach(([aggregateId,aggregateProps]) => this.mainAggregates[aggregateId] = new Aggregate(aggregateProps));
    ...
    // ---------------------------------------------------------------------------------------------------- //
        ...
        this.expenses = FECData.expenses.map((props,index) => new Expense({id: index, ...props}));                                              // external expenses (#60[^3], #61, #62)

        // Stocks ---------------------------------- //

        this.stocks = Object.values(FECData.stocks).map((props,index) => new Stock({id: index, ...props}));                                     // stocks (#31 to #35, #37) / Depreciation (#29)
        this.stockVariations = FECData.stockVariations.map((props,index) => new StockVariation({id: index, ...props}));                         // stock variation (#603, #71)
        ...
        this.immobilisations = Object.values(FECData.immobilisations).map((props,index) => new Immobilisation({id: index, ...props}));          // immobilisations (#20 to #27) / Amortisation (#28) / Depreciation (#29)
        this.amortisationExpenses = FECData.amortisationExpenses.map((props,index) => new AmortisationExpense({id: index, ...props}));          // amortisation expenses (#6811, #6871)
        this.investments = FECData.investments.map((props,index) => new Expense({id: index, ...props}));                                        // investments (flows #2 <- #404)
        this.immobilisedProductions = FECData.immobilisedProductions.map((props,index) => new immobilisedProduction({id: index, ...props}));    // productions of immobilisations (flows #2 <- #72)
    ...
    buildExpensesAccounts = async (periods) =>
    {
        ...
        this.externalExpenseAccounts = this.expenses
            ...
            .map(accountNum => new Account({
                accountNum, 
                accountLib: this.metaAccounts[accountNum].accountLib
            }));
        this.externalExpenseAccounts.forEach(account => account.buildPeriods(this.expenses.filter(expense => expense.accountNum==account.accountNum),periods));
    ...
    buildMainAggregates = async (periods) => 
    {
        ...
        periods.forEach(({periodKey}) => production.periodsData[periodKey] = {
            periodKey,
            amount: roundValue(this.revenue.periodsData[periodKey].amount + this.storedProduction.periodsData[periodKey].amount + this.immobilisedProduction.periodsData[periodKey].amount, 2),
        ...
        let intermediateConsumptions = buildAggregateFromAccounts({
            ...
            accounts: this.externalExpenseAccounts.concat(this.stockVariationAccounts), 
            periods
        });
        ...
        periods.forEach(({periodKey}) => netValueAdded.periodsData[periodKey] = {
            periodKey,
            amount: roundValue(production.periodsData[periodKey].amount - intermediateConsumptions.periodsData[periodKey].amount - fixedCapitalConsumptions.periodsData[periodKey].amount, 2),
            footprint: new SocialFootprint()
        })
        ...
        otherFinancialDataItems.forEach(itemLib => 
        {
            ...
            periods.forEach(period => {
                financialDataItem.periodsData[period.periodKey] = {
                    ...
                    amount: getAmountItems(FECData[itemLib].filter(entry => period.regex.test(entry.date)))
                }
            })
    ...
    {
        this.providers = this.expenses.concat(this.investments)
            .map(expense => {return({
    ...
    initInitialStates = async () =>
    {
        ...
        this.immobilisations
            ...
            .forEach(immobilisation => {
                ...
                let matchingActivity = accountsMatching.branche.filter(({accountRegex}) => (new RegExp(accountRegex)).test(immobilisation.accountNum))[0];
        ...
        this.stocks
            .filter(stock => !stock.isProductionStock)
            .forEach(stock => {
                stock.initialStateType = (this.expenses.some(expense => expense.accountNum.startsWith(stock.expensesAccountsPrefix))) ? "currentFootprint" : "defaultData";
                stock.initialFootprintParams = stock.initialStateType=="defaultData" ? {
                    area: "FRA",
                    code: "00",
                    aggregate: "TRESS",
                } : {};
    ...
    {        
        ...
        // Available accounts
        let prevAccountsData = data.financialData.stocks.concat(data.financialData.immobilisations);

        // Asset accounts
        this.stocks.concat(this.immobilisations)
            .forEach((account) => 
        {
    ...
    getProvider = (providerNum) => this.providers.filter(provider => provider.providerNum == providerNum)[0]
