...
export const analysisTextWriterNRG = (session) =>
{
  ...
  currentParagraph.push("L'intensité énergétique de la valeur produite est de "+printValue(aggregates.production.footprint.indicators.nrg.getValue(),0)+" kJ/€.")
  if (aggregates.production.footprint.indicators.nrg.getValue()!=aggregates.revenue.footprint.indicators.nrg.getValue()) {
    currentParagraph.push("La valeur est de "+printValue(aggregates.revenue.footprint.indicators.nrg.getValue(),0)+" kJ/€ pour le chiffre d'affaires, en prenant compte des stocks de production.")
  } else {
  ...
  else 
  {
    currentParagraph.push("La consommation directe d'énergie est de "+printValue(impactsData.energyConsumption,0)+" MJ,"
      + " soit une intensité de "+printValue(aggregates.netValueAdded.footprint.indicators.nrg.getValue(),0)+" kJ/€ pour la valeur ajoutée.");
  ...
  currentParagraph.push("Les consommations intermédiaires sont à l'orgine d'une consommation indirecte de "+printValue(aggregates.intermediateConsumption.footprint.indicators.nrg.getGrossImpact(aggregates.intermediateConsumption.amount),0)+" MJ,"
    + " ce qui correspond à une intensité de "+printValue(aggregates.intermediateConsumption.footprint.indicators.nrg.getValue(),0)+" kJ/€.");
