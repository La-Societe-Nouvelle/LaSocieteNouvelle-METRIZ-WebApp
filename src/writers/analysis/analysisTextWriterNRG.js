...
export const analysisTextWriterNRG = (session) => {
  ...
  currentParagraph.push(
    "L'intensité énergétique de la valeur produite est de " +
      printValue(aggregates.production.footprint.indicators.nrg.value, 0) +
      " kJ/€."
  );
  if (
    aggregates.production.footprint.indicators.nrg.value !=
    aggregates.revenue.footprint.indicators.nrg.value
  ) {
    currentParagraph.push(
      "La valeur est de " +
        printValue(aggregates.revenue.footprint.indicators.nrg.value, 0) +
  ...
  if (!impactsData.energyConsumption) {
    ...
    currentParagraph.push(
      "La consommation directe d'énergie est de " +
        ...
        printValue(aggregates.netValueAdded.footprint.indicators.nrg.value, 0) +
      ...
      "La consommation directe d'énergie représente " +
        printValue(
          (impactsData.energyConsumption /
            aggregates.production.footprint.indicators.nrg.getGrossImpact(
  ...
  currentParagraph.push(
    "Les consommations intermédiaires sont à l'orgine d'une consommation indirecte de " +
      printValue(
        aggregates.intermediateConsumption.footprint.indicators.nrg.getGrossImpact(
      ...
      printValue(
        aggregates.intermediateConsumption.footprint.indicators.nrg.value,
        0
      ) +
      " kJ/€."
  );
  ...
  currentParagraph.push(
    "L'amortissement des immobilisations représente une consommation indirecte de " +
      printValue(
        aggregates.capitalConsumption.footprint.indicators.nrg.getGrossImpact(
