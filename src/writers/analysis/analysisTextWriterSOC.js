...
export const analysisTextWriterSOC = (session) =>
{
  ...
  currentParagraph.push("L'indicateur informe sur la part de la valeur produite par des entreprises de l'ESS ou ayant défini une raison d'être.");
  ...
  if (impactsData.hasSocialPurpose) 
  {
    ...
    currentParagraph.push("La valeur ajoutée est donc entièrement contributrice aux acteurs d'intérêt social et représente "+printValue(aggregates.netValueAdded.amount/aggregates.production.footprint.indicators.soc.getGrossImpact(aggregates.production.amount)*100,0)+" % de la contribution brute totale.");
  }
  else
  {
    currentParagraph.push("L'entreprise n'est pas dotée d'une raison d'être. L'ensemble des contributions est donc indirect.")
  }
