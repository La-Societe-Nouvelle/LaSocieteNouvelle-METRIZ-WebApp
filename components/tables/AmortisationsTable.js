...
export class AmortisationsTable extends React.Component {
  ...
  render() 
  {
    ...
    const amortisations = depreciations.filter(depreciation => /^28/.test(depreciation.account));
    const amortisationExpenses = depreciationExpenses.filter(expense => /^28/.test(expense.accountAux));
        ...
        <Table  hover>
          ...
          <tbody>
            {immobilisations.map(({account,accountLib}) => {
              let amortisation = amortisations.filter(amortisation => amortisation.accountAux==account)[0];
              if (amortisation!=undefined) {
                ...
                let dimininution = amortisation.prevAmount+augmentation-amortisation.amount;
                return(
                  <tr key={account}>
                    <td >{account}</td>
                    <td>{accountLib.charAt(0).toUpperCase() + accountLib.slice(1).toLowerCase()}</td>
                    <td className="text-end">{printValue(amortisation.prevAmount,0)}  &euro;</td>
                    <td className="text-end">{printValue(augmentation,0)}  &euro;</td>
                    <td className="text-end">{printValue(dimininution,0)}  &euro;</td>
                    <td className="text-end">{printValue(amortisation.amount,0)}  &euro;</td>
                  </tr>)
              } else {
                return(
                  <tr key={account}>
          ...
          {immobilisations.length > 0 &&
             ...
             <tr>
                ...
                <td className="text-end">{printValue(getPrevAmountItems(amortisations),0)} &euro;</td>
                <td className="text-end">{printValue(getAmountItems(amortisationExpenses),0)}  &euro;</td>
                <td className="text-end">{printValue(getPrevAmountItems(amortisations)+getAmountItems(amortisationExpenses)-getAmountItems(amortisations),0)}  &euro;</td>
                <td className="text-end">{printValue(getAmountItems(amortisationExpenses),0)}  &euro;</td>
              </tr>
            </tfoot>
          }
