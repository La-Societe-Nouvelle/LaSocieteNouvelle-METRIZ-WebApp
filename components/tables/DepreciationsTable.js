...
// Utils
import { getAmountItems, getPrevAmountItems, printValue } from '../../src/utils/Utils';
...
export class DepreciationsTable extends React.Component {
  ...
  render() 
  {
    const {immobilisations,depreciations,depreciationExpenses} = this.props.financialData;
    const {columnSorted} = this.state;

    const assetDepreciations = depreciations.filter(depreciation => /^29/.test(depreciation.account));
    const assetDepreciationExpenses = depreciationExpenses.filter(expense => /^29/.test(expense.accountAux));
        ...
        <Table  hover>
          ...
          <tbody>
            {immobilisations.map(({account,accountLib}) => {
              let depreciation = assetDepreciations.filter(depreciation => depreciation.accountAux==account)[0];
              if (depreciation!=undefined) {
                let expenses = assetDepreciationExpenses.filter(expense => expense.accountAux==depreciation.account);
                ...
                return(
                  <tr key={account}>
                    ...
                    <td className="text-end">{printValue(depreciation.prevAmount,0)}  &euro;</td>
                    <td className="text-end">{printValue(augmentation,0)}  &euro;</td>
                    <td className="text-end">{printValue(dimininution,0)}  &euro;</td>
                    <td className="text-end">{printValue(depreciation.amount,0)}  &euro;</td>
                  </tr>)
              } else {
                return(
                  <tr key={account}>
          ...
          {immobilisations.length > 0 &&
             ...
             <tr>
                ...
                <td className="text-end">{printValue(getPrevAmountItems(assetDepreciations),0)} &euro;</td>
                <td className="text-end">{printValue(getAmountItems(assetDepreciationExpenses),0)}  &euro;</td>
                <td className="text-end">{printValue(getPrevAmountItems(assetDepreciations)+getAmountItems(assetDepreciationExpenses)-getAmountItems(assetDepreciations),0)}  &euro;</td>
                <td className="text-end">{printValue(getAmountItems(assetDepreciations),0)}  &euro;</td>
              </tr>
            </tfoot>
          }
        </Table>
